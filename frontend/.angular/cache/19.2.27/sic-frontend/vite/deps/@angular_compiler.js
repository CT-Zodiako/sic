...
function registerContext(ctx, namespace, specs) {
  for (const [element2, attributeNames] of specs) {
    let tagName = namespace && element2 !== "*" && element2 !== "unknown" ? `:${namespace}:${element2}` : element2;
...
function resolveI18nAttrSanitizers(job) {
  const tagNamesByElement = /* @__PURE__ */ new Map();
  for (const unit of job.units) {
    for (const op of unit.ops()) {
...
function ingestElementBindings(unit, op, element2) {
  ...
  for (const attr of element2.attributes) {
    const [ns, elementName] = splitNsName(element2.name);
    let namespace = ns;
    if (!ns) {
      switch (op.namespace) {
        case Namespace.SVG:
...
function compileDeclareClassMetadata(metadata) {
  ...
  definitionMap.set("version", literal("19.2.25"));
