...
@Component({
  ...
  template: `<section class="shell" aria-label="Aplicación autenticada"><aside class="sidebar"><div class="sidebar-brand"><span class="brand-mark" aria-hidden="true">S</span><span class="brand-text"><strong>SIC Colombia</strong><small>Servicios Públicos</small></span></div><div class="sidebar-label">Navegación</div><sic-navigation-menu [nodes]="context.menu" /><div class="sidebar-spacer"></div><button class="sidebar-signout" type="button" (click)="signOut()"><span class="material-symbols-outlined" style="font-size:18px" aria-hidden="true">logout</span> Cerrar sesión</button>
