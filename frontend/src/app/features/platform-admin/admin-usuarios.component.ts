...
@Component({
  ...
  template:
    ...
    `    <mat-card>
          ...
          <section class="detail-section">
            ...
            <div class="detail-rows">
              <div class="detail-row" *ngFor="let membership of userMemberships(user.id)">
                <div><strong>{{companyName(membership.companyId)}}</strong><br><small class="muted" *ngIf="membership.roles?.length">Roles: {{membership.roles!.join(', ')}}</small><small class="muted" *ngIf="!membership.roles?.length">Sin rol asignado</small></div>
      ...
      <div class="create-panel"><form aria-label="Asignar rol a una persona" (submit)="assignRole($event)"><input type="hidden" name="userId" #assignUserId><input type="hidden" name="companyId" #assignCompanyId><input type="hidden" name="roleId" #assignRoleId><mat-form-field appearance="outline"><mat-label>Persona</mat-label><mat-select required (selectionChange)="assignUserId.value = $event.value; selectedAssignUserId = $event.value"><mat-option *ngFor="let user of screen.users" [value]="user.id">{{user.name}} · {{user.email}}</mat-option></mat-select><mat-hint>Quién recibe el rol</mat-hint></mat-form-field><mat-form-field appearance="outline"><mat-label>Empresa</mat-label><mat-select required (selectionChange)="assignCompanyId.value = $event.value; selectedAssignCompanyId = $event.value" [disabled]="!selectedAssignUserId"><mat-option *ngFor="let company of companiesOfUser()" [value]="company.id">{{company.name}}</mat-option></mat-select><mat-hint>{{selectedAssignUserId ? (companiesOfUser().length ? 'Solo sus empresas vinculadas' : 'Esta persona no está vinculada a ninguna empresa') : 'Primero elegí la persona'}}</mat-hint></mat-form-field><mat-form-field appearance="outline"><mat-label>Rol</mat-label><mat-select required (selectionChange)="assignRoleId.value = $event.value" [disabled]="!selectedAssignCompanyId"><mat-option *ngFor="let role of rolesForCompany(selectedAssignCompanyId)" [value]="role.id">{{role.name}} ({{scopeLabel(role)}})</mat-option></mat-select><mat-hint>{{selectedAssignCompanyId ? 'Roles disponibles en esa empresa' : 'Primero elegí la empresa'}}</mat-hint></mat-form-field><button class="btn-create" mat-flat-button color="primary" type="submit" [disabled]="!selectedAssignCompanyId">Asignar rol</button></form><p class="inline-success" *ngIf="lastAssignment" role="status">✓ {{lastAssignment}}</p></div>
