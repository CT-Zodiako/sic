...
@Component({
  ...
  template:
    ...
    `<mat-card aria-labelledby="step-permissions">
      ...
      <div class="create-panel"><form aria-label="Crear función" (submit)="createPermission($event)"><input type="hidden" name="resource" #permissionResourceHidden><mat-form-field appearance="outline"><mat-label>Menú (módulo)</mat-label><mat-select required (selectionChange)="selectPermissionMenu($event.value, permissionResourceHidden)"><mat-option *ngFor="let item of screen.menu" [value]="item.id">{{item.name}}</mat-option></mat-select><mat-hint>La acción se crea para este menú</mat-hint></mat-form-field><label class="tw-field"><span class="tw-field-label">Acción (qué se permite)</span><input class="tw-input" name="action" (input)="updatePermissionPreview()" placeholder="Por ejemplo: ver, crear o borrar"></label><label class="tw-field"><span class="tw-field-label">Código del permiso (se genera solo)</span><input class="tw-input" name="code" [value]="generatedPermissionCode()" readonly><small class="tw-hint">Se compone automáticamente: recurso.acción</small></label><button class="btn-create" mat-flat-button color="primary" type="submit">+ Crear función</button></form><p class="preview-line">Así se compone: <strong>recurso.acción</strong> → <code class="preview-code">{{permissionPreview()}}</code></p><p class="inline-success" *ngIf="lastCreated" role="status">✓ Función <strong>{{lastCreated}}</strong> creada. Aparece en «Funciones existentes» con estado Activo.</p></div>
