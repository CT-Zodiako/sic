...
export class PrismaServiceRepository implements ServiceRepository {
  ...
  private async mutate<T>(mutation: (tx: any) => Promise<T>, event?: AuditEvent): Promise<T> { const work = async (tx: any) => { const result = await mutation(tx); const eventData = result === undefined ? undefined : typeof event === 'function' ? event(result) : event; if (eventData && tx.auditEvent) await tx.auditEvent.create({ data: auditData(eventData) }); return result; }; return this.client.$transaction ? transactional(this.client as any, work) : work(this.client); }
}
